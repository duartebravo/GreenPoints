...
const earnedBadges = [
    { name: "Eco Warrior", description: "Complete 50 ações sustentáveis", color: "bg-emerald-500", icon: "award", points: 100, earnedDate: "Há 3 dias" },
    { name: "Recycler Pro", description: "Recicle 100 itens diferentes", color: "bg-blue-500", icon: "shield", points: 150, earnedDate: "Há 1 semana" },
    { name: "Streak Master", description: "Mantenha uma sequência de 30 dias", color: "bg-orange-500", icon: "flame", points: 200, earnedDate: "Há 2 semanas" },
    { name: "Point Master", description: "Acumule 1000 pontos", color: "bg-purple-500", icon: "star", points: 150, earnedDate: "Há 1 mês" },
];

const lockedBadges = [
    { name: "Transport Hero", description: "Use transporte sustentável por 60 dias", color: "bg-green-500", progress: 35, target: 60, points: 250 },
    { name: "Water Saver", description: "Complete 50 ações de economia de água", color: "bg-cyan-500", progress: 22, target: 50, points: 180 },
        ...
        <div className="min-h-dvh bg-gradient-to-b from-slate-50 to-white pb-20">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 p-6 sm:p-8 text-white">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            <h2 className="text-xl sm:text-2xl font-semibold mb-1 truncate">{user?.name}</h2>
                            <p className="text-sm sm:text-base text-white/90 mb-1 truncate">{user?.email}</p>
                ...
                <div className="p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
                        ...
                        <CardHeader className="pb-3">
                            <CardTitle className="flex items-center justify-between text-amber-700 text-base sm:text-lg">
                                ...
                                <span className="text-sm font-normal text-amber-600">
                                    {user?.badges.length ?? earnedBadges.length}/{earnedBadges.length + lockedBadges.length}
                                </span>
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                                    ...
                                    {earnedBadges.map((badge, index) => {
                                        const renderIcon = (icon: string) => {
                                            switch (icon) {
                                                ...
                                                case "shield": return <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L2 7v10c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V7l-10-5z" /></svg>;
                                                case "flame": return <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M13.5.67s.74 2.65.74 4.8c0 2.06-1.35 3.73-3.41 3.73-2.07 0-3.63-1.67-3.63-3.73l.03-.36C5.21 7.51 4 10.62 4 14c0 4.42 3.58 8 8 8s8-3.58 8-8C20 8.61 17.41 3.8 13.5.67zM11.71 19c-1.78 0-3.22-1.4-3.22-3.14 0-1.62 1.05-2.76 2.81-3.12 1.77-.36 3.6-1.21 4.62-2.58.39 1.29.59 2.65.59 4.04 0 2.65-2.15 4.8-4.8 4.8z" /></svg>;
                                                case "star": return <Star className="w-5 h-5 sm:w-6 sm:h-6 text-white" />;
                                                default: return <Award className="w-5 h-5 sm:w-6 sm:h-6 text-white" />;
                                            }
                                        };

                                        return (
                                            <div key={index} className="flex items-center gap-3 p-3 bg-white rounded-lg border border-emerald-200">
                                                <div className={`w-12 h-12 sm:w-14 sm:h-14 ${badge.color} rounded-full flex items-center justify-center flex-shrink-0 shadow-md`}>
                                                    {renderIcon(badge.icon)}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-sm sm:text-base font-semibold text-slate-900">{badge.name}</p>
                                    ...
                                    {lockedBadges.map((badge, index) => {
                                        ...
                                        return (
                                            <div key={index} className="flex items-start gap-3 p-3 bg-white rounded-lg border border-slate-200">
                                                <div className="w-12 h-12 sm:w-14 sm:h-14 bg-slate-300 rounded-full flex items-center justify-center flex-shrink-0 shadow-md relative">
                                                    <Lock className="w-5 h-5 sm:w-6 sm:h-6 text-slate-500" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-start justify-between gap-2 mb-2">
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-sm sm:text-base font-semibold text-slate-700">{badge.name}</p>
                                                            <p className="text-xs text-slate-600 truncate">{badge.description}</p>
                                                        </div>
                                                        <Badge variant="outline" className="border-slate-400 text-slate-600 flex-shrink-0 text-xs">
                                                            +{badge.points}
                                                        </Badge>
                                                    </div>
                                                    <div className="space-y-1">
                                                        <div className="flex justify-between text-xs">
                                                            ...
                                                            <span className="text-slate-700 font-semibold">{badge.progress}/{badge.target}</span>
                                                        </div>
                    ...
                    {/* Logout Button */}
                    <Button onClick={logout} variant="destructive" className="w-full h-11 sm:h-12 bg-red-600 hover:bg-red-700 shadow-md">
